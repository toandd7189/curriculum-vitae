...
class Securimage {
  ...
  function __construct()
  {
    ...
    if(file_exists(JPATH_BASE.DS.'includes'.DS.'framework.php')){
    	require_once(JPATH_BASE.DS.'includes'.DS.'defines.php');
		require_once(JPATH_BASE.DS.'includes'.DS.'framework.php');
		$mainframe = JFactory::getApplication('site');
		$mainframe->initialise();
		define('CAPTCHA_SESSION_METHOD','JOOMLA');
    } else if ( session_id() == '' ) { // no session has been started yet, which is needed for validation
    	session_start();
		define('CAPTCHA_SESSION_METHOD','ORIGINAL');
    }
  }
  ...
  function saveData()
  {
	...
	if(CAPTCHA_SESSION_METHOD == 'JOOMLA'){
  ...
  function validate()
  {
    ...
    if(CAPTCHA_SESSION_METHOD == 'JOOMLA'){
    	$session = JFactory::getSession();
	    $a = $session->get('securimage_code_value', NULL);
    	$_SESSION['securimage_code_value'] = $a;
    }
	if ( ( isset($_SESSION['securimage_code_value']) OR  isset($_SESSION['__default']['securimage_code_value']))
	  && (!empty($_SESSION['securimage_code_value']) OR !empty($_SESSION['__default']['securimage_code_value'])) ) {
      if ( $_SESSION['securimage_code_value'] == strtolower(trim($this->code_entered)) 
		OR $_SESSION['__default']['securimage_code_value'] == strtolower(trim($this->code_entered))) {
        $this->correct_code = true;
        unset($_SESSION['securimage_code_value']);
		unset($_SESSION['__default']['securimage_code_value']);
      } else {
  ...
  function getCode()
  {
    ...
    } else if (isset($_SESSION['__default']['securimage_code_value']) && !empty($_SESSION['__default']['securimage_code_value'])) {
		return $_SESSION['__default']['securimage_code_value'];
    } else {
      return '';
    }
  }
