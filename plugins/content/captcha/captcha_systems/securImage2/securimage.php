...
class Securimage {
	...
	*/
	...
	function __construct()
	{
		...
		$this->code_length   = 4;
		$this->charset       = 'ABCDEFGHKLMNPRSTUVWYZabcdefghkmnprstuvwyz23456789';
	...
	function saveData()
	{
		...
		if(CAPTCHA_SESSION_METHOD == 'JOOMLA'){
			...
			$session->set('securimage_code_ctime'	,strtolower(time()));
		} else {
	...
	function validate()
	{
		$code               = trim(strtolower($this->getCode()));
	...
	function getCode()
	{
		if(CAPTCHA_SESSION_METHOD == 'JOOMLA'){
			$session = JFactory::getSession();
			if ($this->isCodeExpired($session->get('securimage_code_ctime')) == false) { 
				return strtolower($session->get('securimage_code_value'));
			}
		}elseif (isset($_SESSION['securimage_code_value']) && !empty($_SESSION['securimage_code_value'])) {
	...
	function resetCode(){
		if(CAPTCHA_SESSION_METHOD == 'JOOMLA'){
			$session = JFactory::getSession();
			$session->set('securimage_code_ctime', ''); 
			$session->set('securimage_code_value', '');
		}else{
...
class Securimage_Color {
	 ...
	 */
	function __construct_Color($red, $green = null, $blue = null)
	{
